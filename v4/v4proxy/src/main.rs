...
fn parse_port_mapping(s: &str) -> Option<PortMapping> {
    if let Some(pos) = s.find(':') {
        ...
        Some(PortMapping { local_port, remote_port })
    } else {
        let port = s.trim().parse().ok()?;
        Some(PortMapping { local_port: port, remote_port: port })
    }
}
    ...
    match TcpStream::connect(&target_addr) {
        Ok(server) => {
            ...
            let client_to_server = {
                ...
                thread::spawn(move || {
                    match copy_data(client_read, server, buffer_size) {
                        Ok(bytes) => println!("{} → {}: {} bytes transferred", client_addr, target_addr, bytes),
                        Err(e) => eprintln!("Error in {} → {} direction: {}", client_addr, target_addr, e),
                    }
                })
            };
            ...
            match copy_data(server_read, client, buffer_size) {
                Ok(bytes) => println!("{} → {}: {} bytes transferred", target_addr, client_addr, bytes),
                Err(e) => eprintln!("Error in {} → {} direction: {}", target_addr, client_addr, e),
            }
...
fn start_proxy(
    ...
    println!("Listening on port {} and forwarding to [{}]:{}", 
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                ...
                thread::spawn(move || {
                    if let Err(e) = handle_client(stream, &target, remote_port, timeout_clone, buffer_size_clone) {
...
fn main() -> io::Result<()> {
    ...
    let port_mappings: Vec<PortMapping> = args.ports
        .split(',')
...
mod tests {
    ...
    fn test_port_mapping_equality() {
        let mapping1 = PortMapping { local_port: 8080, remote_port: 25565 };
        let mapping2 = PortMapping { local_port: 8080, remote_port: 25565 };
        let mapping3 = PortMapping { local_port: 9090, remote_port: 25565 };
